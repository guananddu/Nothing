(function() {
        ...
        bFormat: [
        	function(dataItem, i, j, tr, td) {
                ...
                return dataItem.status == 1 
                	? '<input ' + (checked ? 'checked="checked"' : '') + ' _stabletarget_="videoid" videoid="' + dataItem.videoid + '" taskid="' + dataItem.taskid + '" type="checkbox" />' 
                	: '';
            },
            function(dataItem, i, j, tr, td) {
			...
			Lists: function(page, taskid) {
				if(page == undefined) {
                    ...
                    $.get(url, function(o) {
						...
						if(o.status == 200) {
							listsData = o.data;
                            for(var i = 0, len = listsData.length; i < len; i ++) {
                                globalMap[listsData[i].videoid] = listsData[i];
                            }
	...
	var go = (function() {
		...
		return {
			Lists: function(taskId) {
				...
				state = 'Lists';
				pager.setF(state);
				videoIdMap = {}, pushedId = [], videoidAnd = [];
                var page = undefined;
	...
	$('#idsubmit-container').click(function() {
        ...
        for(var i = 0, len = pushedId.length; i < len; i ++) {
            p.push({
                videoid: pushedId[i],
                taskid: globalMap[pushedId[i]].taskid
            });
        }
		$.post(
			'/crawler/addVideo',
